export default {
    "popup-message": "Что такое?",

    "navigation-link-verbs": "Глаголы",
    "navigation-link-numerals": "Números",
    "navigation-link-countries": "Países",
    "navigation-link-words": "Palabras",

    "verbs-header-regular": "(regular)",
    "verbs-header-irregular": "(irregular)",

    "verbs-toolbar-placeholder-search": "Buscar",
    "verbs-toolbar-button-hide": "Чистить",
    "verbs-toolbar-button-show": "Показать",

    "verbs-fields-header-indicative": "Indicativo",
    
    "verbs-footer-select-header": "Выберите",
    "verbs-footer-select-body-1": "Выберите глагол, на выбор 1001 глагол.",
    "verbs-footer-select-body-2": "Глаголы могут быть отсортированы по '100 наиболее распространенным' или в алфавитном порядке.",

    "verbs-footer-study-header": "Готовиться",
    "verbs-footer-study-body-1": "Si quieres ver todas las formas del verbo elegido, haz clic en el botón 'Mostrar'.",
    "verbs-footer-study-body-2": "Si quieres escribir las formas para practicar, haz clic en el botón 'Limpiar' para vaciar el formulario.",
    "verbs-footer-study-body-3": "Puedes utilizar la tecla enter para moverte al siguiente campo y las flechas de dirección del teclado para moverte hacia arriba y abajo.",
    
    "verbs-footer-practice-header": "Практиковать",
    "verbs-footer-practice-body-1": "Limpia el formulario y escribe las formas del verbo para practicar.",
    "verbs-footer-practice-body-2": "Puedes escribir todas las formas o, por ejemplo, solo las formas de indicativo presente.",
    "verbs-footer-practice-body-3": "Cuando escribes, la aplicación corrige cualquier error y sugiere formas alternativas."
};
